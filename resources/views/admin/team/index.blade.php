    ...
    @if($teams->isNotEmpty())
        @foreach ($teams as $index => $team)
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>
                    <a href="#" data-bs-toggle="modal" data-bs-target="#imageModal{{ $team->id }}">
                        <img src="{{ asset($team->image_path) }}" alt="{{ $team->name }}" width="60" height="60" class="rounded-circle" style="cursor: pointer;">
                    </a>

                    <!-- Modal Preview -->
                    <div class="modal fade" id="imageModal{{ $team->id }}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-body text-center">
                                    <img src="{{ asset($team->image_path) }}" class="img-fluid rounded" alt="Preview">
                                </div>
                            </div>
                        </div>
                    </div>
